...
template isNil(t: Thing): bool = not t.exists
...
proc CreateScene(): Scene =
  ...
  scene.lights = @[
    Light(pos: Vector(x: -2.0, y: 2.5, z: 0.0), color: Color(r: 0.49, g: 0.07,
        b: 0.07)),
    Light(pos: Vector(x: 1.5, y: 2.5, z: 1.5), color: Color(r: 0.07, g: 0.07,
        b: 0.49)),
    Light(pos: Vector(x: 1.5, y: 2.5, z: -1.5), color: Color(r: 0.07, g: 0.49,
        b: 0.071)),
    Light(pos: Vector(x: 0.0, y: 3.5, z: 0.0), color: Color(r: 0.21, g: 0.21, b: 0.35))
  ]

  scene.camera = CreateCamera(Vector(x: 3.0, y: 2.0, z: 4.0), Vector(x: -1.0,
      y: 0.5, z: 0.0))
  return scene
...
proc GetNaturalColor(scene: Scene, thing: Thing, pos: Vector, norm: Vector,
    rd: Vector): Color
proc GetReflectionColor(scene: Scene, thing: Thing, pos: Vector, normal: Vector,
...
proc Shade(scene: Scene, isect: Intersection, depth: int): Color =
  ...
  var naturalColor = background.Add(GetNaturalColor(scene, isect.thing, pos,
      normal, reflectDir))
  ...
  if depth >= scene.maxDepth:
    ...
    reflectedColor = GetReflectionColor(scene, isect.thing, pos, normal,
        reflectDir, depth)
...
proc GetPoint(x: int, y: int, camera: Camera, screenWidth: int,
    screenHeight: int): Vector =
...
proc RenderScene(scene: Scene, bitmapData: var seq[RgbColor], stride: int,
    w: int, h: int) =
  var ray: Ray
